...
### 2026-09-14 — 확장 아이콘 클릭을 F4 사이드바와 통일 (background SW 도입)
- 요구: 아이콘 클릭 시 팝업 대신 F4 사이드바가 열려야 함(통일). 앞선 "팝업 유지" 결정을 뒤집어 아이콘=사이드바 토글로
- manifest: `action.default_popup` 제거 → 클릭이 `chrome.action.onClicked` 로 들어옴. `background.service_worker` = `src/background.js` 신규(현재까지 백그라운드 없었음)
...
- 방식 선택: 팝업은 `chrome.tabs`/`scripting.executeScript` 를 써 콘텐츠 스크립트로 그대로 못 옮김 + 크롬 네이티브 사이드패널(`chrome.sidePanel`)은 프로그램적 닫기가 없어 F4 토글이 지저분 → **페이지 내 오버레이(Shadow DOM) + 확장 iframe 으로 popup.html 재사용** 채택(사용자 합의). iframe 은 확장 오리진이라 chrome API 그대로 → popup.js 무포팅 재사용, 백그라운드 SW 불필요
